...
preview_configs = ["_config.yml", "_preview.yml"]
...
task :watch do
  ...
  jekyllPid = Process.spawn({"OCTOPRESS_ENV"=>"preview"}, "jekyll build --watch --config #{preview_configs.join(',')}")
...
task :deploy do
  ...
  if File.exists?(".preview-mode")
    puts "## Found posts in preview mode"
