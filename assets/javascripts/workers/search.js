...
(() => {
  ...
  // src/assets/javascripts/integrations/search/internal/index.ts
  function extract(value, fn) {
    let block = 0;
    let start = 0;
    let end = 0;
  ...
  function tokenizer(input) {
    const tokens = [];
    const segmenter = "TinySegmenter" in lunr ? new lunr.TinySegmenter() : void 0;
            ...
            if (typeof segmenter !== "undefined") {
              ...
              if (/^[MHIK]$/.test(segmenter.ctype_(subsection))) {
                ...
                for (let i = 0, l = 0; i < segments.length; i++) {
                  ...
                  table[block].push(
                    start + index2 + l << 12 | segments[i].length << 2 | type
                  );
  ...
  function highlighter(value, table, positions) {
    const blocks = /* @__PURE__ */ new Map();
    for (const i of positions.sort((a, b) => a - b)) {
      const block = i >>> 20;
      const index2 = i & 1048575;
  ...
  // src/assets/javascripts/integrations/search/query/segment/index.ts
    ...
    constructor({ config, docs, options }) {
      ...
      this.index = lunr(function() {
        ...
        function extractor(field) {
          return (document2) => {
            tables[document2.location][field] = [];
            if (Array.isArray(document2[field])) {
              const values = document2[field];
              return values.map((value) => ({
                toString() {
                  return value;
                },
                table: tables[document2.location][field]
              }));
            } else if (document2[field]) {
              return {
                toString() {
                  return document2[field];
                },
                table: tables[document2.location][field]
              };
            } else {
        ...
        for (const document2 of docs) {
          tables[document2.location] = {};
          this.add(document2, { boost: document2.boost });
        }
      });
    }
    search(query) {
      if (query) {
        try {
          ...
          const groups = this.index.search(query).reduce((item, { ref, score, matchData }) => {
            let document2 = this.documents.get(ref);
            if (typeof document2 !== "undefined") {
              document2 = __spreadValues({}, document2);
              const { tags, parent: parent2 } = document2;
              ...
              for (const [field, positions] of Object.entries(map))
                document2[field] = highlighter(
                  document2[field],
                  this.tables[document2.location][field],
                  positions
                );
              const boost = +!parent2 + Object.values(terms).filter((t) => t).length / Object.keys(terms).length;
              item.push(__spreadProps(__spreadValues(__spreadValues({}, document2), tags && { tags }), {
          ...
          for (const [ref, items] of groups)
            if (!items.find((item) => item.location === ref)) {
              ...
              items.push(__spreadProps(__spreadValues({}, document2), { score: 0, terms: {} }));
            }
          let suggestions;
