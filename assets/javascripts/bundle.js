...
(() => {
  ...
  // node_modules/iframe-worker/polyfill/index.js
  (() => {
    function d(t, e) {
      parent.postMessage(t, e || "*");
    }
    function c(...t) {
      return t.reduce((e, n) => e.then(() => new Promise((r2) => {
        let o = document.createElement("script");
        o.src = n, o.addEventListener("load", () => r2()), document.body.appendChild(o);
      })), Promise.resolve());
    }
    function m() {
      let t = document.createElement("iframe");
      return t.width = t.height = t.frameBorder = "0", t;
    }
    var s = class {
      constructor(e, n) {
        this.url = e;
        ...
        this.handleMessage = (e2) => {
          e2.source === this.worker && (e2.stopImmediatePropagation(), this.dispatchEvent(new MessageEvent("message", { data: e2.data })), this.onmessage && this.onmessage(e2));
        };
        this.handleError = (e2, n2, r3, o, i) => {
          if (n2 === this.url.toString()) {
            let a = new ErrorEvent("error", { message: e2, filename: n2, lineno: r3, colno: o, error: i });
            this.dispatchEvent(a), this.onerror && this.onerror(a);
          }
        };
        if (typeof n != "undefined")
          throw new TypeError("Options are not supported for iframe workers");
        let r2 = new EventTarget();
        this.addEventListener = r2.addEventListener.bind(r2), this.removeEventListener = r2.removeEventListener.bind(r2), this.dispatchEvent = r2.dispatchEvent.bind(r2), document.body.appendChild(this.iframe = m()), this.worker.document.open(), this.worker.document.write(`<html><body><script>postMessage=${d};importScripts=${c};addEventListener("error",ev=>{parent.dispatchEvent(new ErrorEvent("error",{filename:"${e}",error:ev.error}))})<\/script><script src="${e}?${+Date.now()}"><\/script></body></html>`), this.worker.document.close(), window.addEventListener("message", this.handleMessage), window.onerror = this.handleError, this.ready = new Promise((o, i) => {
          this.worker.onload = o, this.worker.onerror = i;
        });
      }
      terminate() {
        document.body.removeChild(this.iframe), window.removeEventListener("message", this.handleMessage), window.onerror = null;
      }
      postMessage(e) {
        this.ready.catch().then(() => {
          this.worker.dispatchEvent(new MessageEvent("message", { data: e }));
        });
      }
      get worker() {
    ...
    window.IFrameWorker = s;
    location.protocol === "file:" && (window.Worker = s);
  })();
  ...
  // src/assets/javascripts/browser/element/hover/index.ts
  ...
  function getElementContainer(el) {
    ...
    while (parent2)
      if (el.scrollWidth <= parent2.scrollWidth && el.scrollHeight <= parent2.scrollHeight)
        parent2 = (el = parent2).parentElement;
      else
        break;
    return parent2 ? el : void 0;
  }
  ...
  function renderAnnotation(id, prefix) {
    ...
    if (prefix) {
      ...
      return /* @__PURE__ */ h("aside", {
        class: "md-annotation",
        tabIndex: 0
      }, renderTooltip(prefix), /* @__PURE__ */ h("a", {
        href: anchor,
      ...
      return /* @__PURE__ */ h("aside", {
        class: "md-annotation",
        tabIndex: 0
      }, renderTooltip(prefix), /* @__PURE__ */ h("span", {
        class: "md-annotation__index",
        tabIndex: -1
      }, /* @__PURE__ */ h("span", {
        "data-md-annotation-id": id
  ...
  function renderSearchDocument(document2, flag) {
    ...
    }, parent2 > 0 && /* @__PURE__ */ h("div", {
      class: "md-search-result__icon md-icon"
    }), parent2 > 0 && /* @__PURE__ */ h("h1", null, document2.title), parent2 <= 0 && /* @__PURE__ */ h("h2", null, document2.title), teaser > 0 && document2.text.length > 0 && document2.text, document2.tags && document2.tags.map((tag) => {
      const type = tags ? tag in tags ? `md-tag-icon md-tag-icon--${tags[tag]}` : "md-tag-icon" : "";
      return /* @__PURE__ */ h("span", {
        class: `md-tag ${type}`
      }, tag);
    }), teaser > 0 && missing.length > 0 && /* @__PURE__ */ h("p", {
  ...
  function renderSearchResultItem(result) {
    ...
    const docs = [...result];
    const parent2 = docs.findIndex((doc) => !doc.location.includes("#"));
    const [article] = docs.splice(parent2, 1);
    ...
    const children = [
      renderSearchDocument(article, 2 /* PARENT */ | +(!parent2 && index2 === 0)),
      ...
      ...more.length ? [
        /* @__PURE__ */ h("details", {
          ...
          tabIndex: -1
        }, /* @__PURE__ */ h("div", null, more.length > 0 && more.length === 1 ? translation("search.result.more.one") : translation("search.result.more.other", more.length))), ...more.map((section) => renderSearchDocument(section, 1 /* TEASER */)))
      ] : []
    ];
  ...
  // src/assets/javascripts/components/tooltip/index.ts
  var sequence = 0;
    ...
    return defer(() => {
      ...
      fromEvent(index2, "mousedown").pipe(
        ...
        if (ev.button !== 0 || ev.metaKey || ev.ctrlKey) {
          ...
          ev.preventDefault();
          const parent2 = el.parentElement.closest(".md-annotation");
          if (parent2 instanceof HTMLElement)
            parent2.focus();
          else
  ...
  function findAnnotationMarkers(container) {
    const markers = [];
    for (const el of container.tagName === "CODE" ? getElements(".c, .c1, .cm", container) : [container]) {
      const nodes = [];
  ...
  function mountAnnotationList(el, container, { target$: target$2, print$: print$2 }) {
    const parent2 = container.closest("[id]");
    const prefix = parent2 == null ? void 0 : parent2.id;
  ...
  // src/assets/javascripts/components/content/annotation/block/index.ts
  ...
  var sequence2 = 0;
  function findCandidateList2(el) {
    if (el.nextElementSibling) {
      const sibling = el.nextElementSibling;
      if (sibling.tagName === "OL")
        return sibling;
      else if (sibling.tagName === "P" && !sibling.children.length)
        return findCandidateList2(sibling);
    }
    return void 0;
  }
  ...
  function mountCodeBlock(el, options) {
    ...
    const factory$ = defer(() => {
      ...
      if (import_clipboard.default.isSupported()) {
        const parent2 = el.closest("pre");
        parent2.id = `__code_${++sequence2}`;
        const button = renderClipboardButton(parent2.id);
        parent2.insertBefore(button, el);
        if (feature("content.tooltips"))
          tooltip = mountTooltip(button);
      }
      const container = el.closest(".highlight");
      if (container instanceof HTMLElement) {
        const list = findCandidateList2(container);
  ...
  function transform(query, fn = (term) => term) {
    ...
    ).join("").replace(/"|(?:^|\s+)[*+\-:^~]+(?=\s+|$)/g, "").split(/\s+/g).flatMap(fn).map((term) => /^[+-]/.test(term) ? term : `${term}*`).join(" ").trim();
  }
  ...
  function watchSearchQuery(el, { rx$ }, { index$: index$2 }) {
    ...
    const value$ = merge(
      ...
      switchMap(() => index$2),
      map(({ config: config4 }) => fn(el.value, (term) => {
        const terms = [];
  ...
  function mountSearchQuery(el, { tx$, rx$ }, options) {
    ...
    return watchSearchQuery(el, { tx$, rx$ }, options).pipe(
      tap((state) => push$.next(state)),
      finalize(() => push$.complete()),
      map((state) => __spreadValues({ ref: el }, state)),
      share()
  ...
  function mountSearchHiglight(el, { index$: index$2, location$: location$2 }) {
    return combineLatest([
      ...
      map(([index2, url]) => setupSearchHighlighter(index2.config)(
  ...
  function watchSidebar(el, { viewport$: viewport$2, main$: main$2 }) {
    const parent2 = el.closest(".md-grid");
    const adjust = parent2.offsetTop - parent2.parentElement.offsetTop;
  ...
  // src/assets/javascripts/patches/ellipsis/index.ts
